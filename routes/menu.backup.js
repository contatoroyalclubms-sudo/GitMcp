...
// Default route - returns menu with products
router.get('/', async (req, res) => {
    try {
        // Return mock data if no menus exist
        const menuData = {
            categories: ['beverages', 'food', 'snacks', 'combos'],
            products: [
                { id: 1, name: 'Coca-Cola', category: 'beverages', price: 5.00, available: true },
                { id: 2, name: 'Beer', category: 'beverages', price: 8.00, available: true },
                { id: 3, name: 'Hot Dog', category: 'food', price: 12.00, available: true },
                { id: 4, name: 'Popcorn', category: 'snacks', price: 7.00, available: true },
                { id: 5, name: 'Combo VIP', category: 'combos', price: 25.00, available: true }
            ]
        };
        res.json(menuData);
    } catch (error) {
