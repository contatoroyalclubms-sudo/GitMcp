...
const seedMenuData = require('./menu-seeder');
...
async function seedDatabase() {
    try {
        ...
        let admin = await User.findOne({ where: { email: 'admin@meep.com' } });
        if (!admin) {
        ...
        let operator = await User.findOne({ where: { email: 'operator@meep.com' } });
        if (!operator) {
        ...
        // Check if we need to seed clients
        ...
        // Check if we need to seed sales
        ...
        if (saleCount === 0) {
            ...
            if (events.length > 0 && clients.length > 0 && products.length > 0) {
                for (let i = 0; i < 20; i++) {
        ...
        // Check if we need to seed transactions
            ...
            if (events.length > 0) {
                await Transaction.bulkCreate([
                    ...
                    },
                    {
                        ...
                        eventId: events[1] ? events[1].id : events[0].id,
                        status: 'completed',
                        paidDate: new Date()
                    }
                ]);
        ...
        try {
            ...
            console.log('⚠️ Menu seeding skipped (model may not exist yet)');
        }
        
        console.log('✅ Database seeding completed successfully');
    } catch (error) {
        console.error('❌ Seeding failed:', error);
        // Don't exit if seeding fails, let the server continue
        // process.exit(1);
    }
}
